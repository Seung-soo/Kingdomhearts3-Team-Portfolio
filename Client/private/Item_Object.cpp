...
CItem_Object::CItem_Object(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
_int CItem_Object::Tick(_double TimeDelta)
{
	if (true == m_bDead)
		return RESULT_DEAD;
	...
	/* 플레이어와의 거리 계산========== */
	_vector Player_Position = m_pGameInstance->Get_Back(current_level, TEXT("Layer_Player"))->Get_Transform()->Get_Position();
	_vector Player_Up = XMVector4Normalize(m_pGameInstance->Get_Back(current_level, TEXT("Layer_Player"))->Get_Transform()->Get_State(CTransform::STATE_UP));
...
_int CItem_Object::LateTick(_double TimeDelta)
{
...
HRESULT CItem_Object::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CItem_Object * CItem_Object::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CItem_Object*	pInstance = new CItem_Object(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CItem_Object");
