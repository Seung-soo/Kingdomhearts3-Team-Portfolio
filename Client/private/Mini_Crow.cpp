...
CMini_Crow::CMini_Crow(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CMini_Crow::CMini_Crow(const CMini_Crow & rhs)
	: CGameObject(rhs)
{
}

HRESULT CMini_Crow::NativeConstruct_Prototype()
{
...
HRESULT CMini_Crow::NativeConstruct(void * pArg)
{
	// 부모 초기화할 때 트랜스폼 초기화
...
_int CMini_Crow::Tick(_double TimeDelta)
{
...
_int CMini_Crow::LateTick(_double TimeDelta)
{
	...
	if (true == bTarget_On && m_pGameInstance->Get_Mouse_Enter(CInput_Device::MBS_LBUTTON))
	{
		...
		_vector	vPos_Crow = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
		_float4 fPos_Crow;
		XMStoreFloat4(&fPos_Crow, vPos_Crow);

		if (FAILED(m_pGameInstance->Add_GameObject(LEVEL_RAPUNZEL_BOSS, TEXT("Layer_Crow"), TEXT("Prototype_GameObject_Good"), &fPos_Crow)))
			return E_FAIL;
...
HRESULT CMini_Crow::SetUp_Components()
{
...
HRESULT CMini_Crow::SetUp_ConstantTable()
{
	_bool bBlur = true;
	...
	_float4		vColor = _float4(0.f, 0.f, 1.f, 0.f);
...
CMini_Crow * CMini_Crow::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CMini_Crow* pInstance = new CMini_Crow(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CMini_Crow");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CMini_Crow::Clone(void * pArg)
{
	CMini_Crow* pInstance = new CMini_Crow(*this);
