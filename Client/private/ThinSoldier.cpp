...
CThinSoldier::CThinSoldier(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CMonster(pDevice, pDeviceContext)
{
}

CThinSoldier::CThinSoldier(const CThinSoldier& rhs)
	: CMonster(rhs)
{
}

HRESULT CThinSoldier::NativeConstruct_Prototype()
{
...
HRESULT CThinSoldier::NativeConstruct(void* pArg)
{
	// 부모 초기화할 때 트랜스폼 초기화
	...
	// 몬스터 타입 설정
	m_MonsterType = MONSTERTYPE_THINSOLDIER;
...
HRESULT CThinSoldier::NativeConstruct_Pool(void * pArg)
{
	//	게임 매니저 몬스터 리스트에 추가
	GM->Add_Monster(this);
...
_int CThinSoldier::Tick(_double TimeDelta)
{
...
_int CThinSoldier::LateTick(_double TimeDelta)
{
	TimeDelta = TimeDelta * m_pStatusCom->Get_SlowSpeed();
...
HRESULT CThinSoldier::Render_Shadow()
{
	if (nullptr == m_pModelCom)
	{
		BREAKPOINT;
...
void CThinSoldier::PostRender(_double TimeDelta)
{
	...
	// 월드행렬 테스트
	pImGui_Manager->SetGameObjectPosition(string("TS Pos"), m_pTransformCom);
	pImGui_Manager->SetGameObjectRotation(string("TS Rot"), m_pTransformCom);
	pImGui_Manager->SetGameObjectScale(string("TS Scale"), m_pTransformCom);
	pImGui_Manager->SmallSeparator();

	// 쉐이더 테스트
	pImGui_Manager->SetGameObjectShaderPass(string("TS Shader"), m_pShaderCom, (_int*)(&m_iPass));
	pImGui_Manager->SmallSeparator();
	...
	// 상태 체크
	pImGui_Manager->CheckMonsterState(string("TS State"), m_pStateMachineCom);
	pImGui_Manager->SmallSeparator();

	ImGui::End();
}

HRESULT CThinSoldier::Save_ObjectData(HANDLE hFile)
{
...
HRESULT CThinSoldier::Load_ObjectData(HANDLE hFile)
{
	if (hFile == INVALID_HANDLE_VALUE)
	{
		BREAKPOINT;
...
_bool CThinSoldier::Damaged(CCollider * pAtkCollider, _float fDamaged)
{
	// 죽기 직전 상태라면 충돌검사 패스
...
HRESULT CThinSoldier::SetUp_Components()
{
	...
	MonsterCoolerDesc.MaxAttackCool      = 7.0;
	...
	ColliderDesc.vScale       = _float3(2.f, 2.f, 2.f);
	ColliderDesc.vTranslation = _float4(0.f, 0.5f, 1.5f, 1.f);
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_TWILIGHT, TEXT("Prototype_Component_Model_ThinSoldier"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;

	return S_OK;
}

HRESULT CThinSoldier::SetUp_RootNodeIndex()
{
...
HRESULT CThinSoldier::SetUp_Animations()
{
...
HRESULT CThinSoldier::SetUp_ConstantTable()
{
	// 림라이트
	if (m_iPass == 12)
	{
...
void CThinSoldier::OnCollision(_double TimeDelta)
{
	// 죽기 직전 상태라면 충돌검사 패스
	if (m_bReadyToDie == true)
		return;
...
void CThinSoldier::ChangeState(_double TimeDelta)
{
	...
	if (m_pStateMachineCom->FindMonsterState() == false)
	{
		// 쿨타임이 되는 경우에만 DynamicMove 상태 생성하고 쿨타임 초기화
...
CThinSoldier* CThinSoldier::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CThinSoldier* pInstance = new CThinSoldier(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CThinSoldier");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CThinSoldier::Clone(void* pArg)
{
	CThinSoldier* pInstance = new CThinSoldier(*this);
