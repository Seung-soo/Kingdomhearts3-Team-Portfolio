...
class CNobody final : public CMonster
{
private:
	CNobody(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	CNobody(const CNobody& rhs);
	virtual ~CNobody() = default;
...
private:
	...
	CCollider*		m_pAtkColliderCom[2] = { nullptr, };
...
// 피격
	...
	_matrix					m_AtkCollider_SocketMatrix[2];
	_matrix					m_AtkCollider_TransformMatrix[2];

public:
	static	CNobody* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
