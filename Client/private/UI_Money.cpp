...
HRESULT CUI_Money::NativeConstruct(void * pArg)
{
	...
	SetX = 128.f + fGap + NM->Get_MoneyPos();
	SetY = 280.f;
	m_fDepth = 0.19f; // ±íÀÌ
...
HRESULT CUI_Money::SetUp_Components()
{
	...
	/* For.Com_Texture */
	CHECK_FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_Money_Font"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom));

	return S_OK;
}

HRESULT CUI_Money::SetUp_ConstantTable()
{
...
CUI_Money * CUI_Money::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	...
	{
		MSGBOX("Failed to Created CUI_Money");
