...
HRESULT CMonkey::NativeConstruct(void* pArg)
{
	...
	// 몬스터 타입 설정
	m_MonsterType = MONSTERTYPE_MONKEY;
...
HRESULT CMonkey::NativeConstruct_Pool(void * pArg)
{
	//	게임 매니저 몬스터 리스트에 추가
	GM->Add_Monster(this);
...
_int CMonkey::Tick(_double TimeDelta)
{
...
_int CMonkey::LateTick(_double TimeDelta)
{
	TimeDelta = TimeDelta * m_pStatusCom->Get_SlowSpeed();
...
void CMonkey::PostRender(_double TimeDelta)
{
	...
	// 월드행렬 테스트
	pImGui_Manager->SetGameObjectPosition(string("MK Pos"), m_pTransformCom);
	pImGui_Manager->SetGameObjectRotation(string("MK Rot"), m_pTransformCom);
	pImGui_Manager->SetGameObjectScale(string("MK Scale"), m_pTransformCom);
	pImGui_Manager->SmallSeparator();

	// 쉐이더 테스트
	pImGui_Manager->SetGameObjectShaderPass(string("MK Shader"), m_pShaderCom, (_int*)(&m_iPass));
	pImGui_Manager->SmallSeparator();
	...
	// 상태 체크
	pImGui_Manager->CheckMonsterState(string("MK State"), m_pStateMachineCom);
	pImGui_Manager->SmallSeparator();

	ImGui::End();
}

HRESULT CMonkey::Save_ObjectData(HANDLE hFile)
{
...
HRESULT CMonkey::Load_ObjectData(HANDLE hFile)
{
	if (hFile == INVALID_HANDLE_VALUE)
	{
		BREAKPOINT;
...
_bool CMonkey::Damaged(CCollider * pAtkCollider, _float fDamaged)
{
	// 죽기 직전 상태라면 충돌검사 패스
...
HRESULT CMonkey::SetUp_Components()
{
	...
	MonsterCoolerDesc.MaxDynamicMoveCool = 10.0; 
	MonsterCoolerDesc.MaxChaseCool       = 5.0;
	...
	ColliderDesc.vTranslation = _float4(0.f, 0.75f, 0.75f, 1.f);
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_RAPUNZEL, TEXT("Prototype_Component_Model_Monkey"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;

	return S_OK;
}

HRESULT CMonkey::SetUp_RootNodeIndex()
{
...
HRESULT CMonkey::SetUp_Animations()
{
...
void CMonkey::OnCollision(_double TimeDelta)
{
	// 죽기 직전 상태라면 충돌검사 패스
	if (m_bReadyToDie == true)
		return;
...
void CMonkey::ChangeState(_double TimeDelta)
{
