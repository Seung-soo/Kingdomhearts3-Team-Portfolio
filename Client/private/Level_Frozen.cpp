...
CLevel_Frozen::CLevel_Frozen(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CLevel(pDevice, pDeviceContext)
{
}

HRESULT CLevel_Frozen::NativeConstruct()
{
	...
	m_pRenderer->Set_LUTDesc(true, 95);
	m_pRenderer->Set_HDR(true);
	m_pRenderer->Set_Fog(true);
	m_pRenderer->Set_FogStart(0.f);
	m_pRenderer->Set_FogEnd(0.1f);
	m_pRenderer->Set_FogColor(_float4(0.5f, 0.5f, 0.9f, 1.f));
	m_pRenderer->Set_MaxFogFactor(0.5f);
	m_pRenderer->Set_LUTTexIndexSecond(95);
	m_pRenderer->Set_DOF(true);

	//m_pGameInstance->PlayBGM(TEXT("FrozenBGM.mp3"), 0.6f);
...
_int CLevel_Frozen::Tick(_double TimeDelta)
{
...
_int CLevel_Frozen::LateTick(_double TimeDelta)
{
...
HRESULT CLevel_Frozen::Ready_Lights()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	...
	LightDesc.vDiffuse   = _float4(0.9f, 0.9f, 1.f, 1.f);
	LightDesc.vAmbient   = _float4(0.4f, 0.4f, 0.5f, 1.f);
...
HRESULT CLevel_Frozen::Ready_Layer_GameObjects()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_Camera"), TEXT("Prototype_GameObject_Camera_Perspective"), &CameraDesc)))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	GM->Set_Camera(static_cast<CCamera_Perspective*>(pGameInstance->Get_Back(LEVEL_FROZEN, TEXT("Layer_Camera"))));

	// BackGround
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_BackGround"), TEXT("Prototype_GameObject_Sky"))))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	...
	// Player
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_Player"), TEXT("Prototype_GameObject_Player"))))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	GM->Set_Player(static_cast<CPlayer*>(m_pGameInstance->Get_Back(LEVEL_FROZEN, TEXT("Layer_Player"))));
	GM->Get_Player()->Set_ControlMode(true);

	// IceMan
	if (nullptr == pGameInstance->ThrowToLayer_And_Return(LEVEL_FROZEN, TEXT("Layer_IceMan"), nullptr))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	...
	// Icicle
	CFileLoad_Manager::GetInstance()->Load_SaveFile(CFileLoad_Manager::SAVEFILE_FZ_ICICLES);


	return S_OK;
}

HRESULT CLevel_Frozen::Ready_Layer_UI()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);

	// UI :: Player_Circle* ==================================================================================================================================
	_int iCom_Texture_Set = CUI_Parents::UI_CIRCLE::UI_CIRCLE_MAIN;
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_UI"), TEXT("Prototype_GameObject_UI_Player_Circle"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_UI"), TEXT("Prototype_GameObject_UI_Target"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_UI"), TEXT("Prototype_GameObject_UI_bt_Command"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_UI"), TEXT("Prototype_GameObject_TextCommand"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_UI"), TEXT("Prototype_GameObject_FriendIcon"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FROZEN, TEXT("Layer_UI"), TEXT("Prototype_GameObject_TextQuickSlot"), &iCom_Texture_Set)))
		return E_FAIL;
...
CLevel_Frozen* CLevel_Frozen::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CLevel_Frozen* pInstance = new CLevel_Frozen(pDevice, pDeviceContext);
