...
cbuffer RadialBlur
{
	static const float g_fRadialBlurSamples[11] = {
...
PS_OUT PS_MAIN_RADIALBLUR(PS_IN In)
{
	...
	for (int i = 0; i < 11; i++)
	{
		float2 vOffset = In.vTexUV - vDir * g_fRadialBlurSamples[i] * g_fBlurDist;

		vOffset = clamp(vOffset, float2(0.f, 0.f), float2(1.f, 1.f));

		vSum += g_TargetTexture.Sample(ClampSampler, vOffset);
	}
	vSum /= 11.f;
...
PS_OUT PS_MAIN_FOG(PS_IN In)
{
	...
	float		fFogFactor = saturate(saturate((g_fFogEnd - fDistance) 
		/ (g_fFogEnd - g_fFogStart)) + g_fMaxFogFactor);
