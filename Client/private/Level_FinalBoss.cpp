...
CLevel_FinalBoss::CLevel_FinalBoss(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CLevel(pDevice, pDeviceContext)
{
}

HRESULT CLevel_FinalBoss::NativeConstruct()
{
	...
	m_pRenderer->Set_LUTDesc(true, 96);
	m_pRenderer->Set_LUTTexIndexSecond(96);

	//m_pGameInstance->PlayBGM(TEXT("FinalBossBGM.mp3"), 0.5f);
...
_int CLevel_FinalBoss::Tick(_double TimeDelta)
{
...
_int CLevel_FinalBoss::LateTick(_double TimeDelta)
{
...
HRESULT CLevel_FinalBoss::Ready_Layer_GameObjects()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_Camera"), TEXT("Prototype_GameObject_Camera_Perspective"), &CameraDesc)))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	GM->Set_Camera(static_cast<CCamera_Perspective*>(pGameInstance->Get_Back(LEVEL_FINALBOSS, TEXT("Layer_Camera"))));

	// BackGround
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_BackGround"), TEXT("Prototype_GameObject_Sky"))))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	...
	// Player
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_Player"), TEXT("Prototype_GameObject_Player"))))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	GM->Set_Player(static_cast<CPlayer*>(m_pGameInstance->Get_Back(LEVEL_FINALBOSS, TEXT("Layer_Player"))));
	pTransform = GM->Get_Player()->Get_Transform();
	pTransform->Set_Position(__vector(-9.f, 0.f, 0.f, 1.f));
	pTransform->MakeSameDirection(__vector(1.f, 0.f, 0.f, 0.f));

	// Goat
	if (nullptr == pGameInstance->ThrowToLayer_And_Return(LEVEL_FINALBOSS, TEXT("Layer_Goat"), nullptr))
	{
		BREAKPOINT;
		return E_FAIL;
	}
	pTransform = static_cast<CTransform*>(m_pGameInstance->Get_Component(LEVEL_FINALBOSS, TEXT("Layer_Goat"), TEXT("Com_Transform")));
	pTransform->Set_Position(__vector(9.f, 0.f, 0.f, 1.f));
	pTransform->MakeSameDirection(__vector(-1.f, 0.f, 0.f, 0.f));
...
HRESULT CLevel_FinalBoss::Ready_Layer_UI()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);

	// UI :: Player_Circle* ==================================================================================================================================
	_int iCom_Texture_Set = CUI_Parents::UI_CIRCLE::UI_CIRCLE_MAIN;
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_UI"), TEXT("Prototype_GameObject_UI_Player_Circle"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	iCom_Texture_Set = 0;
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_UI"), TEXT("Prototype_GameObject_UI_Player_RageGauge"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_UI"), TEXT("Prototype_GameObject_UI_Target"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_UI"), TEXT("Prototype_GameObject_UI_bt_Command"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_UI"), TEXT("Prototype_GameObject_TextCommand"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_UI"), TEXT("Prototype_GameObject_FriendIcon"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_FINALBOSS, TEXT("Layer_UI"), TEXT("Prototype_GameObject_TextQuickSlot"), &iCom_Texture_Set)))
		return E_FAIL;
	...
	MINIGAME->Set_ENDLoading(true);

	return S_OK;

}

CLevel_FinalBoss* CLevel_FinalBoss::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CLevel_FinalBoss* pInstance = new CLevel_FinalBoss(pDevice, pDeviceContext);
