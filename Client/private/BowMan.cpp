...
CBowMan::CBowMan(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CMonster(pDevice, pDeviceContext)
{
}

CBowMan::CBowMan(const CBowMan& rhs)
	: CMonster(rhs)
{
}

HRESULT CBowMan::NativeConstruct_Prototype()
{
...
HRESULT CBowMan::NativeConstruct(void* pArg)
{
	// 부모 초기화할 때 트랜스폼 초기화
	...
	// 몬스터 타입 설정
	m_MonsterType = MONSTERTYPE_BOWMAN;
...
HRESULT CBowMan::NativeConstruct_Pool(void * pArg)
{
	//	게임 매니저 몬스터 리스트에 추가
	GM->Add_Monster(this);
...
_int CBowMan::Tick(_double TimeDelta)
{
...
_int CBowMan::LateTick(_double TimeDelta)
{
	TimeDelta = TimeDelta * m_pStatusCom->Get_SlowSpeed();
...
void CBowMan::PostRender(_double TimeDelta)
{
	...
	// 월드행렬 테스트
	pImGui_Manager->SetGameObjectPosition(string("BM Pos"), m_pTransformCom);
	pImGui_Manager->SetGameObjectRotation(string("BM Rot"), m_pTransformCom);
	pImGui_Manager->SetGameObjectScale(string("BM Scale"), m_pTransformCom);
	pImGui_Manager->SmallSeparator();

	// 쉐이더 테스트
	pImGui_Manager->SetGameObjectShaderPass(string("BM Shader"), m_pShaderCom, (_int*)(&m_iPass));
	pImGui_Manager->SmallSeparator();

	// 상태 체크
	pImGui_Manager->CheckMonsterState(string("BM State"), m_pStateMachineCom);
	pImGui_Manager->SmallSeparator();

	ImGui::End();
}

HRESULT CBowMan::Save_ObjectData(HANDLE hFile)
{
...
HRESULT CBowMan::Load_ObjectData(HANDLE hFile)
{
	if (hFile == INVALID_HANDLE_VALUE)
	{
		BREAKPOINT;
...
_bool CBowMan::Damaged(CCollider * pAtkCollider, _float fDamaged)
{
	// 죽기 직전 상태라면 충돌검사 패스
...
HRESULT CBowMan::SetUp_Components()
{
	...
	ColliderDesc.vTranslation = _float4(0.f, 0.75f, 0.f, 1.f);
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_TWILIGHT, TEXT("Prototype_Component_Model_BowMan"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;

	return S_OK;
}

HRESULT CBowMan::SetUp_RootNodeIndex()
{
...
HRESULT CBowMan::SetUp_Animations()
{
...
HRESULT CBowMan::SetUp_ConstantTable()
{
	// 월드행렬
	if (FAILED(m_pTransformCom->Bind_OnShader(m_pShaderCom, "g_WorldMatrix")))
		return E_FAIL;
...
void CBowMan::OnCollision(_double TimeDelta)
{
	// 죽기 직전 상태라면 충돌검사 패스
	if (m_bReadyToDie == true)
		return;
	...
	for (auto& pDestCollider : *pColliderList)
	{
		...
		if (m_pGameInstance->Collision_Sphere(m_pHitColliderCom, pDestCollider))
		{
			...
			// 역방향으로 밀려나기
			m_pTransformCom->Go_VectorXZ(-vDir, TimeDelta, nullptr);
		}
	}
}

void CBowMan::ChangeState(_double TimeDelta)
{
...
CBowMan* CBowMan::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CBowMan* pInstance = new CBowMan(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CBowMan");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CBowMan::Clone(void* pArg)
{
	CBowMan* pInstance = new CBowMan(*this);
