#include "stdafx.h"
#include "..\public\UI_ComboBar.h"
#include "GameInstance.h"
#include "UI_Num_Manager.h"
#include "GameManager.h"

CUI_ComboBar::CUI_ComboBar(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CUI_Parents(pDevice, pDeviceContext)
{
}

CUI_ComboBar::CUI_ComboBar(const CUI_ComboBar & rhs)
	: CUI_Parents(rhs)
{
}

HRESULT CUI_ComboBar::NativeConstruct_Prototype()
{
...
HRESULT CUI_ComboBar::NativeConstruct(void * pArg)
{
...
_int CUI_ComboBar::Tick(_double TimeDelta)
{
	if (true == m_bDead)
		return RESULT_DEAD;
	...
	SetX = 1100.f + NM->Get_ComboBarPos();
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(SettingX + SetX, SettingY + SetY, 0.f, 1.f));

	return RESULT_NOPROBLEM;
}

_int CUI_ComboBar::LateTick(_double TimeDelta)
{
...
HRESULT CUI_ComboBar::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Texture */
	CHECK_FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Copmonent_Texture_ComboBar"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom));

	return S_OK;
}

HRESULT CUI_ComboBar::SetUp_ConstantTable()
{
...
CUI_ComboBar * CUI_ComboBar::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CUI_ComboBar*	pInstance = new CUI_ComboBar(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CUI_ComboBar");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CUI_ComboBar::Clone(void * pArg)
{
	CUI_ComboBar*	pInstance = new CUI_ComboBar(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX("Failed to Created CUI_ComboBars");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CUI_ComboBar::Free()
{
	__super::Free();
