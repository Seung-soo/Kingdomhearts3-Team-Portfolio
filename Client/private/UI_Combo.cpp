#include "stdafx.h"
#include "..\public\UI_Combo.h"
#include "GameInstance.h"
#include "UI_Num_Manager.h"
#include "GameManager.h"

CUI_Combo::CUI_Combo(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
HRESULT CUI_Combo::NativeConstruct(void * pArg)
{
	...
	SetX = 1200.f + fGap + NM->Get_ComboPos();
	SetY = 250.f;
	m_fDepth = 0.19f; // ±íÀÌ
...
_int CUI_Combo::Tick(_double TimeDelta)
{
...
_int CUI_Combo::LateTick(_double TimeDelta)
{
...
CUI_Combo * CUI_Combo::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CUI_Combo*	pInstance = new CUI_Combo(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CUI_Combo");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CUI_Combo::Clone(void * pArg)
{
